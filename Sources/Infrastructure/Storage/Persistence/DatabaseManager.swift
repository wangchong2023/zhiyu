...
final class DatabaseManager {
    ...
    func setup(at dbURL: URL) throws {
        // 核心修复：更精确的内存数据库检测，支持 URI 参数 (mode=memory)
        let urlString = dbURL.absoluteString
        let isMemory = urlString.contains(":memory:") || urlString.contains("mode=memory")
        ...
        if let current = dbWriter {
            ...
            // 内存模式下比对完整 URI，文件模式下比对路径
            if isMemory {
                if currentPath == ":memory:" || urlString.contains(currentPath) {
                    return
                }
            } else if currentPath == dbURL.path {
                return
            }
        }
        ...
        if isMemory {
            // 对于内存数据库，直接初始化一个私有的 DatabaseQueue
            // 这在测试环境下是最安全且最快的，且自动支持并发测试（每个实例互不干扰）
            self.dbWriter = try DatabaseQueue(configuration: config)
        } else {
            // 使用 DatabasePool 启用 WAL 模式，提升文件数据库的并发性能
