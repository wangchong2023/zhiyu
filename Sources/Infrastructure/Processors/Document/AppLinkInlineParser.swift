...
//
//  系统层级：[L1] 基础设施层
//  核心职责：智宇双链感知的行内富文本解析器。
//           基于两阶段解析算法，优先保护 `[[双链]]` 不被贪婪正则匹配破坏，
//           随后在夹缝区间级联提取加粗、斜体、删除线、外链及行内代码等标准格式。
//
import Foundation

// MARK: - 双链感知行内解析器

/// 智宇系统核心行内解析器，负责将单行文本拆解为带格式标记的 `InlineSegment` 序列。
/// 该解析器独立于块级解析，可被 swift-markdown 或自研块解析器复用。
struct AppLinkInlineParser: Sendable {
...
extension NSRegularExpression {
    static let appLinkRegex: NSRegularExpression = {
        ...
        catch { fatalError("AppLinkInlineParser: 无效正则 appLinkRegex — \(error)") }
    }()
    static let boldRegex: NSRegularExpression = {
        do { return try NSRegularExpression(pattern: "(?<!\\\\)\\*\\*(.+?)\\*\\*") }
        catch { fatalError("AppLinkInlineParser: 无效正则 boldRegex — \(error)") }
    }()
    static let italicRegex: NSRegularExpression = {
        do { return try NSRegularExpression(pattern: "(?<!\\\\)[\\*_](.+?)[\\*_]") }
        catch { fatalError("AppLinkInlineParser: 无效正则 italicRegex — \(error)") }
    }()
    static let strikethroughRegex: NSRegularExpression = {
        do { return try NSRegularExpression(pattern: "(?<!\\\\)~~(.+?)~~") }
        catch { fatalError("AppLinkInlineParser: 无效正则 strikethroughRegex — \(error)") }
    }()
    static let codeRegex: NSRegularExpression = {
        do { return try NSRegularExpression(pattern: "(?<!\\\\)`(.+?)`") }
        catch { fatalError("AppLinkInlineParser: 无效正则 codeRegex — \(error)") }
    }()
    static let linkRegex: NSRegularExpression = {
        do { return try NSRegularExpression(pattern: "(?<!\\\\)\\[(.+?)\\]\\((.+?)\\)") }
        catch { fatalError("AppLinkInlineParser: 无效正则 linkRegex — \(error)") }
    }()
}
