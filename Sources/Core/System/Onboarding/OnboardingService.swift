...
//
// 作者: Wang Chong
// 功能说明: [L0.5] 系统集成层：本文件实现了知识管理系统的新手引导服务（OnboardingService），旨在为新用户提供沉浸式的产品价值呈现与交互教学。
...
final class OnboardingService: ObservableObject {
    ...
    init() {
        ...
        if UserDefaults.standard.object(forKey: "hasCompletedOnboarding") != nil {
            let old = UserDefaults.standard.bool(forKey: "hasCompletedOnboarding")
            UserDefaults.standard.set(old, forKey: AppConstants.Keys.Storage.hasCompletedOnboarding)
            UserDefaults.standard.removeObject(forKey: "hasCompletedOnboarding")
            self.hasCompletedOnboarding = old
        } else {
