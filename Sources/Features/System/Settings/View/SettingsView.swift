...
//
// 作者: Wang Chong
// 功能说明: [L2] 业务功能层：本文件定义了“设置”页面，负责管理应用的外观（主题/语言）、AI 配置、数据同步/备份以及安全隐私设置。
// 核心功能点：
...
struct SettingsView: View {
    ...
    private func exportAllAsMarkdown() -> String {
        ...
        for type in PageType.allCases {
            let typePages = store.pages.filter { $0.pageType == type }
            ...
            for page in typePages.sorted(by: { $0.title < $1.title }) {
                output += "---\n\n"
                output += "### \(page.title)\n\n"
                output += "- \(L10n.Transfer.tr("export.type")): \(page.pageType.displayName)\n"
                ...
                output += "- \(L10n.Transfer.tr("export.created")): \(page.createdAt.formatted(Date.FormatStyle(locale: Localized.currentLocale)))\n"
                output += "- \(L10n.Transfer.tr("export.updated")): \(page.updatedAt.formatted(Date.FormatStyle(locale: Localized.currentLocale)))\n\n"
                output += page.content
                output += "\n\n"
            }
        }
