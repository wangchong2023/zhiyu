...
final class AppStore: @preconcurrency GraphDataProvider {
    ...
    var growthSeries: [KnowledgeGrowthPoint] {
        ...
        for daysAgo in (0...AppUI.Metrics.knowledgeGrowthDaysLimit).reversed() { // 30
    ...
    func createPage(
        ...
        if totalPages >= AppUI.Metrics.graphCoachMarkThreshold && !settingsStore.hasShownGraphCoachMark { // 3
            settingsStore.hasShownGraphCoachMark = true
            Task {
                try? await Task.sleep(nanoseconds: UInt64(AppUI.Action.animationDuration * 5 * 1_000_000_000)) // 1s delay
                await MainActor.run {
...
extension AppStore {
    ...
    func loadPDFDocuments() async -> [PDFDocumentInfo] {
        await PDFProcessor.shared.loadDocumentsInfo()
    }

    /// 将 PDF 原始数据保存至本地存储。
    func savePDFDocument(data: Data, fileName: String) async -> URL? {
        await PDFProcessor.shared.savePDF(data: data, fileName: fileName)
    }

    /// 持久化 PDF 文档元数据列表。
    func savePDFDocuments(_ docs: [PDFDocumentInfo]) async {
        await PDFProcessor.shared.saveDocumentsInfo(docs)
    }
