...
/// - Warning: 本枚举已弃用。请迁移至 `Sources/Shared/DesignSystem/Tokens/` 下的原子令牌。
/// 集成了原子常数、间距规范、图标体系、组件指标及视觉装饰等核心 UI 令牌。
@available(*, deprecated, message: "请使用 DesignSystem/Tokens 下的 Spacing, Typography, Colors, Animations 分层令牌")
enum AppUI {
    ...
    /// 动态背景与氛围光生成 (已迁移至 Sources/Shared/UIComponents/Backgrounds/PageBackground.swift)
    
    // MARK: - 22. 玻璃拟态组件 (Glass-morphism Components)
    /// 统一的玻璃卡片修饰符 (已迁移至 Sources/Shared/UIComponents/Modifiers/GlassStyle.swift)
    
    /// 标准卡片容器 (已迁移至 Sources/Shared/UIComponents/Layouts/StandardSection.swift)
    typealias AppSection<Content: View> = StandardSection<Content>
}

// MARK: - View 扩展
extension View {
    /// 应用玻璃拟态卡片样式 (已迁移至 Sources/Shared/UIComponents/Modifiers/GlassStyle.swift)
    
    /// 应用列表行样式 (用于 AppSection 内部) (已迁移至 Sources/Shared/UIComponents/Layouts/StandardSection.swift)
}


// MARK: - 语义化颜色与扩展 (已迁移至 Sources/Shared/DesignSystem/Tokens/Colors.swift)
// 此处仅保留 typealias 以维持兼容性，具体实现请参考 Colors.swift

// MARK: - View 容器扩展 (部分已迁移至 Sources/Shared/UIComponents/Layouts/StandardSection.swift)
extension View {
}
