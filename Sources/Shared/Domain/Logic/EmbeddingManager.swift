...
final class EmbeddingManager {
    ...
    /// 同步所有待更新的页面向量
    func syncEmbeddings(pages: [KnowledgePage]) {
    ...
    /// 当单个页面更新时触发
    func updateEmbedding(for page: KnowledgePage) {
        syncQueue.async { [weak self] in
            guard let self = self, let model = self.embeddingModel else { return }
            let text = "\(page.title)\n\(page.content.prefix(1000))"
    ...
    func indexChunks(pageID: UUID, chunks: [PageChunk]) {
        syncQueue.async { [weak self] in
            guard let self = self, let model = self.embeddingModel else { return }
    ...
    func search(query: String, topK: Int = AppConfig.AI.topKResults) -> [(id: UUID, score: Float)] {
        ...
        let results = performParallelSearch(queryVector: qv, keys: keys, cache: currentCache)
    ...
    func multiQuerySearch(query: String, topK: Int = AppConfig.AI.topKResults) async -> [(chunk: PageChunk, score: Float)] {
        ...
        for q in allQueries {
            if let qv = vectorize(text: q) {
                let results = searchChunks(queryVector: qv, topK: 50)
    ...
    func hydeSearch(query: String, topK: Int = AppConfig.AI.topKResults) async -> [(chunk: PageChunk, score: Float)] {
        ...
        let results = searchChunks(queryVector: qv, topK: topK)
    ...
    private func searchChunks(queryVector: [Float], topK: Int) -> [(id: String, score: Float)] {
        let currentCache: [String: [Float]] = {
            lock.lock()
            defer { lock.unlock() }
            return chunkVectorCache
        }()
        
        let keys = Array(currentCache.keys)
        let results = performParallelSearch(queryVector: queryVector, keys: keys, cache: currentCache)
        return results.sorted { $0.1 > $1.1 }.prefix(topK).map { $0 }
    }
    
    private func performParallelSearch<K>(queryVector: [Float], keys: [K], cache: [K: [Float]]) -> [(K, Float)] {
