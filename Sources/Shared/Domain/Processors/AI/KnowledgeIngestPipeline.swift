...
//
// 作者: Wang Chong
// 功能说明: 本文件实现了知识摄入管道（KnowledgeIngestPipeline），作为 RAG (Retrieval-Augmented Generation) 系统的核心编排层。
// 该管道通过统一的接口将内容增强、文本分块与向量索引三个独立模块串联起来，实现了“数据 -> 知识 -> 索引”的自动化流水线。
// 核心职责：
// 1. 流程编排：依次调用 AIContentEnricher (语义增强) -> TextChunkerProcessor (语义分块) -> VectorIndexer (向量化)。
// 2. 状态同步：确保分块结果与 KnowledgePage 的元数据保持一致。
// 3. 错误处理：管理流水线中的异常情况，确保局部失败不影响整体摄入。
// 版本: 1.0
// 修改记录:
//   - 2026-05-06: 初始创建，实现 RAG 流程的标准化编排
