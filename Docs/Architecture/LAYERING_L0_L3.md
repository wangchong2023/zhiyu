...
| **物理多库隔离** | 🚀 进行中 | 重构 `DatabaseManager` 支持动态 `switchDatabase(to:)` 物理切换，解除单库强寄生。 |
| **全局库化下沉** | 🚀 进行中 | 建立全局配置库 `global.sqlite3` 结构化托管笔记本卡片列表、防篡改签名表等全局数据。 |
| **依赖倒置 (DIP)** | ✅ 已修复 | 所有的仓储协议已物理下沉至 L1.5 (Domain) 层，实现彻底解耦。 |
| **AppStore 治理** | ✅ 已修复 | `AppStore` 已完成瘦身，标签、统计、PDF 等逻辑已迁移至专有领域 Store。 |
| **安全与常量治理**| 🚀 进行中 | 补全 `SecurityManager` 核心 API 三斜杠中文注释，消除短字 `4` 等魔鬼数字并收拢全局盐值。 |
