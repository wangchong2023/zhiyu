...
> **遗留压测规模增强计划 (十万节点压测)**：目前的性能压测与回归防线覆盖了 50,000+ 级别的页面节点。由于用户可能会通过级联捕获引擎（CaptureCascadeEngine）及外置代理批量导入大量碎知识点，后续版本发布前，必须在混沌压测套件中增加 **100,000 节点级别** 的高吞吐量写入与检索压力测试，验证 SQLite 的 FTS5 性能收敛性。
